MaxCoverage
===========

.. currentmodule:: ioh.iohcpp.problem

.. autoclass:: MaxCoverage
   :show-inheritance:

   .. rubric:: Attributes Summary

   .. autosummary::

      ~MaxCoverage.bounds
      ~MaxCoverage.constraints
      ~MaxCoverage.log_info
      ~MaxCoverage.meta_data
      ~MaxCoverage.optimum
      ~MaxCoverage.problems
      ~MaxCoverage.state

   .. rubric:: Methods Summary

   .. autosummary::

      ~MaxCoverage.__call__
      ~MaxCoverage.add_constraint
      ~MaxCoverage.attach_logger
      ~MaxCoverage.create
      ~MaxCoverage.detach_logger
      ~MaxCoverage.enforce_bounds
      ~MaxCoverage.load_instances
      ~MaxCoverage.remove_constraint
      ~MaxCoverage.reset
      ~MaxCoverage.set_id
      ~MaxCoverage.set_instance
      ~MaxCoverage.set_name

   .. rubric:: Attributes Documentation

   .. autoattribute:: bounds
   .. autoattribute:: constraints
   .. autoattribute:: log_info
   .. autoattribute:: meta_data
   .. autoattribute:: optimum
   .. autoattribute:: problems
   .. autoattribute:: state

   .. rubric:: Methods Documentation

   .. automethod:: __call__
   .. automethod:: add_constraint
   .. automethod:: attach_logger
   .. automethod:: create
   .. automethod:: detach_logger
   .. automethod:: enforce_bounds
   .. automethod:: load_instances
   .. automethod:: remove_constraint
   .. automethod:: reset
   .. automethod:: set_id
   .. automethod:: set_instance
   .. automethod:: set_name
