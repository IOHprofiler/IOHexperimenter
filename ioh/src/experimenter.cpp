...
void define_experimenter(py::module &m, const std::string &name)
{
    ...
    py::class_<Experimenter>(m, name.c_str(), py::buffer_protocol())
        ...
        .def("run", &Experimenter::run, "Perform the benchmarking procedure.")
...
{
    define_experimenter<ioh::problem::Real>(m, "Real", "An experimenter object for use with continuous optimization problems (e.g. BBOB).");
    define_experimenter<ioh::problem::Integer>(m, "Integer", "An experimenter object for use with discrete optimization problems (e.g PBO).");
}
