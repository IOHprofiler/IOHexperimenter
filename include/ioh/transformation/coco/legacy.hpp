...
#define SUITE_BBOB2009_MAX_DIM 40
			...
			static void bbob2009_unif(std::vector<double>& r, long N, long inseed)
			{
				...
				for (i = 39; i >= 0; i--)
				{
					tmp = static_cast<int>(floor(static_cast<double>(aktseed) / static_cast<double>(127773)));
				...
				for (i = 0; i < N; i++)
				{
					...
					if (aktseed < 0)
						aktseed = aktseed + 2147483647;
					tmp = static_cast<int>(floor(static_cast<double>(aktrand) / static_cast<double>(67108865)));
					aktrand = rgrand[tmp];
					rgrand[tmp] = aktseed;
			 ...
			 */
			static void bbob2009_reshape(std::vector<std::vector<double>>& B, const std::vector<double>& vector,
			...
			static void bbob2009_gauss(std::vector<double>& g, const size_t N, const long seed)
			{
				...
				assert(2 * N < 6000);
				bbob2009_unif(uniftmp, 2L * N, seed);
			...
			static void bbob2009_copy_rotation_matrix(const std::vector<std::vector<double>>& rot,
			...
			static double bbob2009_compute_fopt(const size_t function, const size_t instance)
			{
				...
				rrseed = rseed + static_cast<long>(10000 * instance);
				bbob2009_gauss(gval, 1, rrseed);
				bbob2009_gauss(gval2, 1, rrseed + 1);
				return bbob2009_fmin(
					1000., bbob2009_fmax(-1000., bbob2009_round(100. * 100. * gval[0] / gval2[0]) / 100.));
			}
		}
	}
}
