...
namespace ioh
{
	namespace logger
	{
		...
		public:
			...
			void track_problem(int problem_id, int dimension, int instance, std::string problem_name,
			                   common::optimization_type maximization_minimization_flag);

			void track_problem(const problem::base<int>& problem) override;

			void track_problem(const problem::base<double>& problem) override;
		...
		private:
			...
			// std::shared_ptr<problem<int> >  tracked_problem_int;
			// std::shared_ptr<problem<double> > tracked_problem_double;
		};
	}
}
