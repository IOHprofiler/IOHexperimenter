...
namespace ioh::problem::funnel
{
    ...
    class DoubleSphere final : public RealSingleObjective
    {
        double d_;
        double s_;
        double u1_;
    ...
    protected:
        double evaluate(const std::vector<double> &x) override { return functions::double_sphere(x, u1_, u2_, s_, d_); }
    public:
        /*
         ...
         */
        DoubleSphere(const int n_variables, const double d = 0.0, const double s = 1.0) :
            RealSingleObjective(MetaData(0, 1, "DoubleSphere", n_variables), Bounds<double>(n_variables, -5, 5), {},
