	...
	namespace problem
	{
		namespace bbob
		{
			class Gallagher101 : public bbob_base
			{
				...
				static int f_gallagher_compare_doubles1(const void* a, const void* b_)
				{
					auto temp = (*(f_gallagher_permutation_t1*)a).value - (*(f_gallagher_permutation_t1*)b_).value;
					if (temp > 0)
						return 1;
					if (temp < 0)
						return -1;
			...
			public:
				...
				void prepare_problem() override
				{
					...
					* probability, not the largest condition level!!! */
					...
					free(rperm);
					rperm = static_cast<f_gallagher_permutation_t1*>(malloc((n_) * sizeof(f_gallagher_permutation_t1)));
					...
					for (i = 0; i < number_of_peaks; ++i)
					{
						...
						//std::sort(rperm.begin(), rperm.end(), compareRperm);
						qsort(rperm, n_, sizeof(f_gallagher_permutation_t1), f_gallagher_compare_doubles1);
						for (j = 0; j < n_; ++j)
						{
