...
namespace ioh {
    namespace problem {
        namespace bbob {
            ...
            public:
                Schaffers1000(int instance_id = IOH_DEFAULT_INSTANCE,
                ...
                void prepare_problem() override {
                    ...
                    for (auto i = 0; i < n_; ++i) {
                        b_[i] = 0.0;
                        for (auto j = 0; j < n_; ++j) {
                            const auto exponent = 1.0 * static_cast<int>(i) / (
                                                      static_cast<double>(
                                                          static_cast<long>(n_))
                                                      -
                                                      1.0);
                            m_[i][j] = rot2_[i][j] * pow(
                                           sqrt(conditioning_), exponent);
                        }
                    }
                }
                ...
                double internal_evaluate(
                    ...
                    return pow(
                        result / (
                            static_cast<double>(static_cast<long>(n_)) - 1.0),
                        2.0);
                }

                static Schaffers1000 *create(
