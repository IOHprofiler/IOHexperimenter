	...
	namespace problem
	{
		namespace bbob
		{
			class Gallagher21 : public bbob_base
			{
				...
				{
					auto temp = (*(f_gallagher_permutation_t*)a).value - (*(f_gallagher_permutation_t*)b).value;
					if (temp > 0)
						return 1;
					if (temp < 0)
						return -1;
			...
			public:
				Gallagher21(int instance_id = DEFAULT_INSTANCE, int dimension = DEFAULT_DIMENSION)
					: bbob_base(22, "Gallagher21", instance_id)
				{
				...
				) override
				{
					...
					* probability, not the largest condition level!!! */
					...
					free(rperm);
					rperm = static_cast<f_gallagher_permutation_t*>(malloc((n) * sizeof(f_gallagher_permutation_t)));
