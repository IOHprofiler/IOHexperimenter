	...
	namespace problem
	{
		namespace bbob
		{
			class Gallagher21 : public bbob_base
			{
				...
				static bool f_gallagher_compare_doubles(const f_gallagher_permutation_t& a, const f_gallagher_permutation_t& b)
				{
			...
			public:
				...
				void prepare_problem() override
				{
					...
					const double f1 = 1.1, f2 = 9.1, b_ = 9.8, c = 4.9;

					std::vector<double> random_numbers;
					std::vector<double> arr_condition(number_of_peaks, maxcondition);
					std::vector<f_gallagher_permutation_t> rperm(number_of_peaks - 1);
					...
					std::sort(rperm.begin(), rperm.end(), f_gallagher_compare_doubles);

					/* Random permutation */
					for (auto i = 1; i < number_of_peaks; ++i)
					{
						const auto rank = rperm[i - 1].index / static_cast<double>(number_of_peaks - 2);
						arr_condition[i] = pow(maxcondition, rank);
						peak_values[i] = static_cast<double>(i - 1) / static_cast<double>(number_of_peaks - 2) * (f2 - f1) + f1;
					}
					...
					for (auto i = 0; i < number_of_peaks; ++i)
					{
						...
						for (auto j = 0; j < n_; ++j)
							arr_scales[i][j] = pow(arr_condition[i], 
							                       rperm[j].index / static_cast<double>(n_ - 1) -0.5);
					}
				...
				double internal_evaluate(const std::vector<double>& x) override
				{
					...
					auto a = 0.1;
					double f = 0., f_add = 0., f_true = 0.;
					const double fac = -0.5 / static_cast<double>(n_);
					...
					for (auto i = 0; i < number_of_peaks; ++i)
					{
						double tmp2 = 0.;
						for (auto j = 0; j < n_; ++j)
							tmp2 += arr_scales[i][j] * pow((tmx[j] - x_local[j][i]), 2.0);
