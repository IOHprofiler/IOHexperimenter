...
namespace ioh {
    namespace problem {
        namespace bbob {
            ...
            public:
                Weierstrass(int instance_id = IOH_DEFAULT_INSTANCE,
                ...
                void prepare_problem() override {
                            ...
                            for (auto k = 0; k < n_; ++k) {
                                ...
                                const auto exponent =
                                    1.0 * static_cast<int>(k) / (
                                        static_cast<double>(static_cast<
                                            long>(n_)) - 1.0
                                    );
                                m_[i][j] += rot1_[i][k] * pow(base, exponent) *
                                    rot2_[k][j];
                            }
                        }
                    }
                    ...
                    for (auto i = 0; i < summands; ++i) {
                        ...
                        f0 += ak[i] * cos(
                            2 * transformation::coco::coco_pi * bk[i] * 0.5);
                    }
                }
                ...
                double internal_evaluate(
                    ...
                    return 10.0 * pow(
                               result / static_cast<double>(static_cast<long>(n_
                               )) - f0,
                               3.0);
                }
                ...
                void objectives_transformation(const std::vector<double> &x,
                    ...
                    transformation::coco::transform_obj_penalize_evaluate(
                        raw_x_, lower_bound_, upper_bound_,
                        penalty_factor_, y);
                }
