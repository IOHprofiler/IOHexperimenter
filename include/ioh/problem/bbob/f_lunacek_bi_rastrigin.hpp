...
namespace ioh {
    namespace problem {
        namespace bbob {
            ...
            public:
                Lunacek_Bi_Rastrigin(int instance_id = IOH_DEFAULT_INSTANCE,
                ...
                double internal_evaluate(
                    ...
                    const auto s =
                        1. - 0.5 / (sqrt(static_cast<double>(n_ + 20)) - 4.1);
                    ...
                    for (i = 0; i < n_; ++i) {
                        tmpvect[i] = 0.0;
                        const auto c1 = pow(sqrt(condition),
                                            static_cast<double>(i) / static_cast
                                            <double>(n_ - 1));
                        for (j = 0; j < n_; ++j) {
                    ...
                    return std::min(
                               sum1, d * static_cast<double>(n_) + s * sum2)
                           + 10. * (static_cast<double>(n_) - sum3) + 1e4 *
                           penalty;
                }

                static Lunacek_Bi_Rastrigin *create(
