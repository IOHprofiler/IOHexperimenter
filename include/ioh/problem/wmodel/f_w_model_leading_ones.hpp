    ...
    namespace problem {
        namespace wmodel {
            ...
            public:
                ...
                W_Model_LeadingOnes(int instance_id = IOH_DEFAULT_INSTANCE,
                                    int dimension = IOH_DEFAULT_DIMENSION)
                    : wmodel_base("W_Model_LeadingOnes", instance_id,
                                  dimension),
                      temp_dimension(0) {
                ...
                void prepare_problem() override {
                    this->temp_dimension = static_cast<int>(this->
                        get_number_of_variables());

                    if (this->dummy_para > 0) {
                        this->dummy_info = utils::dummy(
                            this->temp_dimension, dummy_para, 10000);
                        assert(
                            this->dummy_info.size() == static_cast<size_t>(this
                                ->temp_dimension *
                                this->dummy_para));
                        this->temp_dimension = static_cast<int>(this->dummy_info
                            .size());
                    }

                    if (this->neutrality_para > 0) {
                        this->temp_dimension =
                            this->temp_dimension / this->neutrality_para;
                    }
                    if (this->ruggedness_para > 0) {
                        this->ruggedness_info = utils::ruggedness_raw(
                            utils::ruggedness_translate(this->ruggedness_para,
                                this->temp_dimension),
                            this->temp_dimension);
                    }
                }
                ...
                void customize_optimal() override {
                    const auto dimension = get_number_of_variables();
                    const auto optimal_value = static_cast<int>(
                                                   dimension * (
                                                       this->dummy_para == 0
                                                           ? 1
                                                           : this->dummy_para))
                ...
                void set_w_setting(const double dummy_para,
                                   const int epistasis_para,
                                   const int neutrality_para,
                ...
                double internal_evaluate(const std::vector<int> &x) override {
                    ...
                    if (this->neutrality_para > 0) {
                        tempX = w_model_x;
                        utils::layer_neutrality_compute(
                            tempX, w_model_x, this->neutrality_para);
                    }

                    // Epistasis layer
                    if (this->epistasis_para > 0) {
                        tempX = w_model_x;
                        utils::layer_epistasis_compute(
                            tempX, w_model_x, this->epistasis_para);
                    }
                ...
                static W_Model_LeadingOnes *create(
