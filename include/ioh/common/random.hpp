	...
	namespace common
	{
		...
		class Random
		{
			...
			void generate()
			{
				...
				{
					auto t = this->x_[i] + this->x_[i + (IOH_LONG_LAG - IOH_SHORT_LAG)];
					if (t >= 1.0)
						t -= 1.0;
					this->x_[i] = t;
				}
			...
			static long lcg_rand(long seed)
			{
				...
				seed = static_cast<long>(IOH_RND_MULTIPLIER * (
					seed - seed_mod * IOH_RND_MODULUS_DIV) - IOH_RND_MOD_MULTIPLIER * seed_mod);
		...
		public:
			 ...
			 */
			explicit Random(uint32_t seed = IOH_DEFAULT_SEED) : seed_index_(0), x_{0}
			{
