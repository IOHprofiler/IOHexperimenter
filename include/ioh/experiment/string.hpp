	...
	namespace experiment
	{
		...
		static std::vector<int> get_int_vector_parse_string(std::string input, const int _min, const int _max)
		{
			...
			for (size_t i = 0; i < n; ++i)
			{
				...
				{
					common::log::error("The configuration consists invalid characters.");
				}
			}
			...
			for (size_t i = 0; i < n; ++i)
			{
				...
				if (spiltstring[i][0] == '-')
				{
					...
					if (i != 0)
					{
						common::log::error("Format error in configuration.");
					}
					else
					{
						...
						if (tmpvalue < _min)
						{
							common::log::error("Input value exceeds lowerbound.");
						}
				...
				else if (spiltstring[i][spiltstring[i].length() - 1] == '-')
				{
					...
					else
					{
						...
						if (tmpvalue > _max)
						{
							common::log::error("Input value exceeds upperbound.");
						}
