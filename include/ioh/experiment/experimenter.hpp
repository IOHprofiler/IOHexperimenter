	...
	namespace experiment
	{
		...
		public:
			...
			void run()
			{
				...
				while ((current_problem_ = suite_->get_next_problem()) != nullptr)
				{
					...
					for (auto count = 0; independent_runs_ > count; ++count)
					{
						...
						std::cout << "." << std::flush;
						current_problem_ = suite_->get_current_problem(); // This resets problem
					}
				}
			}
			 ...
			 */
			std::shared_ptr<suite::base<ProblemType>> get_suite() const
			{
				return suite_;
			}
