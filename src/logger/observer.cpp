...
namespace ioh
{
	namespace logger
	{
        ...
        void observer::set_complete_flag(bool complete_flag) {
            this->observer_complete_flag = complete_flag;
        }

        bool observer::complete_status() const {
            return observer_complete_flag;
        }

        bool observer::complete_trigger() const {
            return observer_complete_flag;
        }

        void observer::set_interval(int interval) {
            this->observer_interval = interval;
        }

        bool observer::interval_status() const {
        ...
        bool observer::interval_trigger(size_t evaluations) const {
            if (observer_interval == 0) {
                return false;
            }
        ...
        void observer::set_update_flag(bool update_flag) {
            this->observer_update_flag = update_flag;
        }

        bool observer::update_status() const {
            return observer_update_flag;
        }

        bool observer::update_trigger(double fitness, common::optimization_type optimization_type) {
            if (observer_update_flag == false) {
                return false;
            }

            if (common::compare_objectives(fitness, current_best_fitness, optimization_type)) {
        ...
        void observer::set_time_points(const std::vector<int>& time_points, const int number_of_evaluations, const int time_points_exp_base1, const int time_points_exp_base2) {
        ...
        bool observer::time_points_status() const {
        ...
        bool observer::time_points_trigger(size_t evaluations) {
        ...
        void observer::reset_observer(const  common::optimization_type optimization_type) {
