/**
 * This file implements an evolutionary algorithm with self-adaptive mutation operator,
 * which uses the log-normal update rule for adapting the mutation rate.
 * Initialized mutation rate is set as 0.2.
 * Size of parents: 1
...
size_t mutateIndividual(int * individual, 
  ...
  l = randomBinomial(dimension,mutation_rate + 1.0/(double)dimension,random_generator);
  while(l == 0){
    l = randomBinomial(dimension,mutation_rate + 1.0/(double)dimension,random_generator);
  }
...
void User_Algorithm(evaluate_function_t evaluate,
  ...
  int hit_optimal = 0;
  int lambda = 10;
  double temp_mutation_rate = 0.2;
  double best_mutation_rate = 0.2;
  double mutation_rate;
  double tempb;
  ...
  for (i = 1; i < max_budget; ) {
    ...
    for(j = 0; j < lambda; ++j){
      ...
      temp_mutation_rate = 1.0 / (1.0 + (((1.0 - mutation_rate) / mutation_rate) * exp(0.22*IOHprofiler_random_normal(random_generator))));
      l = mutateIndividual(offspring,dimension,temp_mutation_rate,random_generator);
      
      p[0] = lambda; p[1] = temp_mutation_rate + 1.0/(double)(dimension); p[2] = (double)l;
      set_parameters(number_of_parameters,p);
      ...
      if(y[0] >= best_value){
        best_mutation_rate = temp_mutation_rate;
