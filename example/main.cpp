...
// }
//
// // double sphere(const Eigen::VectorXf& x)
// // {
// //     return x.dot(x);
// // }
//
...
// void time_vec()
// {
//     const std::vector<double> xv(100, 2);
//     ioh::common::CpuTimer x;
//     auto r = 0.;
...
// void time_eigen()
// {
//     auto xe = Eigen::Matrix<float, 100, 1>::Ones(100) * 2.;
//     ioh::common::CpuTimer x;
//     auto r = 0.;
//     for (int i = 0; i < 1000000; i++)
//         r += sphere(xe);
...
int main() {
    ...
    const auto items = {
         the_factory.create("Sphere", 1, 2),
         the_factory.create("Ellipsoid", 1, 2),
         // the_factory.create(D3::id, 2, 3),
         // the_factory.create(D2::id, 3, 3),
         // the_factory.create(D2::id, 4, 2),
         // the_factory.create(D3::id, 5, 2),
         // the_factory.create(D1::id, 5, 3)
   }; 
   
    // Use the items we got from the factory
    ...
    //
    // auto f = wrap_function<double>(
    //     &null_function, "Nuller", 5);
    //
    // std::cout << f << std::endl;
    // f(std::vector<double>(5, 0));
    // std::cout << f << std::endl;
    //
    // f.meta_data();
    // f.state().evaluations;
