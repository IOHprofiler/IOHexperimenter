/**
 * @file f_ising_one_dim.c
 * @brief Implementation of the ising_one_dim function and problem.
 */
...
void change_ising_one_dim_representation(int* from, int* to, int number_of_variables){
...
int modulo_ising_one_dim(int x,int N){
    return (x % N + N) %N;
}
 ...
 */
static int f_ising_one_dim_raw(const int *x, const size_t number_of_variables) {
    size_t i = 0;
    int result, sum_neig;
    int x_tmp[number_of_variables];
    change_ising_one_dim_representation(x, x_tmp, number_of_variables);
    ...
    for (i = 0; i < number_of_variables; ++i) {
        sum_neig=x_tmp[modulo_ising_one_dim((i+1), number_of_variables)] + x_tmp[modulo_ising_one_dim((i -1) , number_of_variables)];
        result += 2 * x_tmp[i] * sum_neig;
    }
 ...
 * @brief Uses the raw function to evaluate the IOHprofiler problem.
 */
static void f_ising_one_dim_evaluate(IOHprofiler_problem_t *problem, const int *x, double *y) {
    assert(problem->number_of_objectives == 1);
    y[0] = f_ising_one_dim_raw(x, problem->number_of_variables);
    assert(y[0] <= problem->best_value[0]);
}

/**
 * @brief Allocates the basic ising_one_dim problem.
 */
static IOHprofiler_problem_t *f_ising_one_dim_allocate(const size_t number_of_variables) {
    IOHprofiler_problem_t *problem = IOHprofiler_problem_allocate_from_scalars("ising_one_dim function",
                                                                               f_ising_one_dim_evaluate, NULL, number_of_variables, 0, 1, 1);

    IOHprofiler_problem_set_id(problem, "%s_d%02lu", "ising_one_dim", number_of_variables);

    /* Compute best solution */
    f_ising_one_dim_evaluate(problem, problem->best_parameter, problem->best_value);
    return problem;
}

/**
 * @brief Creates the IOHprofiler ising_one_dim problem.
 */
static IOHprofiler_problem_t *f_ising_one_dim_IOHprofiler_problem_allocate(const size_t function,
    ...
    problem = f_ising_one_dim_allocate(dimension);
